...
=== PLAN ===
[attachment 2: .plan.json @ plan-2026-09-15-intent-record-merge-main-into-fe -- 3721 bytes, not copied]
```
...
[
  ...
  {"id": "item-6", "quote": "Add the merge changelog entry under docs/changes/. Add this merge's line to HISTORY.md. Update NEXT-STEPS.md. File after-observations for L1 C1 A1 A2 M1 M2 T1 ST1 I1 under docs/methodology/_pending/."},
  {"id": "item-7", "quote": "Conflicts are resolved by a coder subagent under this approved plan: the human-granted builder subagent (coder spawn 2, 2026-09-15) does steps 2-6; the Overseer does not edit those files."},
  ...
  {"id": "obs-C1-no-conflict-markers", "quote": "Strip all conflict markers."},
  {"id": "obs-A1-amh-one-refill", "quote": "keep this branch's def hydrate_customers(token: str, item: dict) -> dict; delete main's list-taking definition and the ThreadPoolExecutor import; in main() keep only per-item build_wo(hydrate_customers(token, item)) (bulk loop and single-WO path), never hydrate_customers(token, [; keep main's build_wo warnings including the exact string no contact (phone/name) on this WO and the no-street warning."},
  {"id": "obs-A2-amh-parses", "quote": "Run C1, A1, A2, M1, M2 exactly as in the intent record; all must exit 0."},
  {"id": "obs-M1-msr-open-tab-scan", "quote": "keep this branch's find-new handler (open list tab only; must include scanned host list; must not include find-new: via=); keep isMSRListPage(); keep main's ping handler with onList: isMSRListPage(); keep loadInIframe(url, kind) for capture paths."},
  {"id": "obs-M2-msr-alarm-and-bail", "quote": "keep main's const POLL_MINUTES = 1, get-then-create alarm at top level, and drain on worker spawn; delete this branch's periodInMinutes: 0.5; keep function isMsrListUrl; find-new bail uses this branch's list-aware message plus main's find-new: BAIL log."},
  ...
  {"id": "obs-I1-suite-stays-green", "quote": "I1 (node test/run.js; exit 0 and contains \", 0 fail\")."}
]
```
