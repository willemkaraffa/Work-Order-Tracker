...
=== PLAN ===
[attachment 2: .plan.json @ plan-2026-09-15-intent-record-merge-main-into-fe -- 3292 bytes, not copied]
```
...
[
  ...
  {"id": "item-6", "quote": "Append this merge's line to HISTORY.md (existing style). Update NEXT-STEPS.md to the remaining user live checks (one MSR Find new on an open list tab; one Capture all AMH). Write docs/changes/2026-09-15-merge-main-into-schedule-retention.md."},
  {"id": "item-7", "quote": "MISSING"},
  {"id": "obs-L1-po-pinned-294bac8", "quote": "Confirm L1 (dependency ends with #294bac8 and lock packages['node_modules/project-overseer'].resolved ends with #294bac8dabd1c4296f28e7a0fe079e83caff2232)."},
  {"id": "obs-C1-no-conflict-markers", "quote": "Run the intent C1/A1/A2/M1/M2 one-liners (not verifyBudget)."},
  {"id": "obs-A1-amh-one-refill", "quote": "Run the intent C1/A1/A2/M1/M2 one-liners (not verifyBudget)."},
  {"id": "obs-A2-amh-parses", "quote": "Run the intent C1/A1/A2/M1/M2 one-liners (not verifyBudget)."},
  {"id": "obs-M1-msr-open-tab-scan", "quote": "Run the intent C1/A1/A2/M1/M2 one-liners (not verifyBudget)."},
  {"id": "obs-M2-msr-alarm-and-bail", "quote": "Run the intent C1/A1/A2/M1/M2 one-liners (not verifyBudget)."},
  {"id": "obs-T1-smoke-test", "quote": "Run T1: node test/renderer-smoke.test.js (expect exit 0 and ALL PASS)."},
  {"id": "obs-ST1-status-enforcing", "quote": "Then ST1: node node_modules/project-overseer/scripts/overseer-status.js (expect exit 0, contains 'gates ARE enforcing', lacks GAP)."},
  {"id": "obs-I1-suite-stays-green", "quote": "Then I1: node test/run.js (expect exit 0 and ', 0 fail')."}
]
```
