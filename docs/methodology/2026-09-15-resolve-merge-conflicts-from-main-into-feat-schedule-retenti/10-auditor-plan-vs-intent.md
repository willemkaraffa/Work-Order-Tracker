...
[
  {
    "id": "item-1",
    "quote": "Resolve scrape_amh.py: keep this branch's def hydrate_customers(token: str, item: dict) -> dict; delete main's list-taking definition and the ThreadPoolExecutor import; in main() keep only per-item build_wo(hydrate_customers(token, item)) (bulk loop and single-WO path), never hydrate_customers(token, [; keep main's build_wo warnings including the exact string no contact (phone/name) on this WO and the no-street warning."
  },
  {
    "id": "item-2",
    "quote": "Resolve extension/content.js: keep this branch's find-new handler (open list tab only; must include scanned host list; must not include find-new: via=); keep isMSRListPage(); keep main's ping handler with onList: isMSRListPage(); keep loadInIframe(url, kind) for capture paths."
  },
  {
    "id": "item-3",
    "quote": "Resolve extension/background.js: keep main's const POLL_MINUTES = 1, get-then-create alarm at top level, and drain on worker spawn; delete this branch's periodInMinutes: 0.5; keep function isMsrListUrl; find-new bail uses this branch's list-aware message plus main's find-new: BAIL log."
  },
  {
    "id": "item-4",
    "quote": "Resolve test/renderer-smoke.test.js by keeping this branch's version (clear App intervals, stub fetch, assert error boundary did not catch, seed Admin S1 note migration). Drop main's duplicate window list and watchdog."
  },
  {
    "id": "item-5",
    "quote": "Run npm install github:willemkaraffa/Project-Overseer#6fdba72. No other package.json change. Confirm L1: package.json pin ends with #6fdba72, lock packages[node_modules/project-overseer].resolved ends with #6fdba72d00f4ab658496174103feefcc85784c6d, and node_modules/project-overseer/package.json version is 0.16.0."
  },
  {
    "id": "item-6",
    "quote": "Add the merge changelog entry under docs/changes/. Add this merge's line to HISTORY.md. Update NEXT-STEPS.md. File after-observations for L1 C1 A1 A2 M1 M2 T1 ST1 I1 under docs/methodology/_pending/."
  },
  {
    "id": "item-7",
    "quote": "MISSING"
  ...
  {
    "id": "obs-L1-po-pinned-6fdba72",
    "quote": "Confirm L1: package.json pin ends with #6fdba72, lock packages[node_modules/project-overseer].resolved ends with #6fdba72d00f4ab658496174103feefcc85784c6d, and node_modules/project-overseer/package.json version is 0.16.0."
  },
  {
    "id": "obs-C1-no-conflict-markers",
    "quote": "Run C1, A1, A2, M1, M2 exactly as in the intent record; all must exit 0."
  },
  {
    "id": "obs-A1-amh-one-refill",
    "quote": "Run C1, A1, A2, M1, M2 exactly as in the intent record; all must exit 0."
  },
  {
    "id": "obs-A2-amh-parses",
    "quote": "Run C1, A1, A2, M1, M2 exactly as in the intent record; all must exit 0."
  },
  {
    "id": "obs-M1-msr-open-tab-scan",
    "quote": "Run C1, A1, A2, M1, M2 exactly as in the intent record; all must exit 0."
  },
  {
    "id": "obs-M2-msr-alarm-and-bail",
    "quote": "Run C1, A1, A2, M1, M2 exactly as in the intent record; all must exit 0."
  },
  {
    "id": "obs-T1-smoke-test",
    "quote": "Run T1 (node test/renderer-smoke.test.js; exit 0 and contains ALL PASS)"
  },
  {
    "id": "obs-ST1-status-enforcing",
    "quote": "Run ST1 (node node_modules/project-overseer/scripts/overseer-status.js; exit 0, contains gates ARE enforcing, lacks GAP)."
  },
  {
    "id": "obs-I1-suite-stays-green",
    "quote": "then I1 (node test/run.js; exit 0 and contains \", 0 fail\")."
  }
]
```
