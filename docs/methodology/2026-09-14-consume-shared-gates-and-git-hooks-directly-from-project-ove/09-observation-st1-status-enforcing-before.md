...
PLAN
  plan-2026-09-08-admin-s5-slice-j4-wo-listing-and
  status: done  (not enforced; only an approved plan constrains edits)
  goal: Admin S5 slice J4: WO listing and sort in the Journal rail's Clients tree. Spec is roadmap-handoffs/admin-s5-journal-ux.md:333. Today clientTree (src/orders-logic.js:1067) builds all three levels from NOTES, so a property lists only the WOs that carry a user note. J4 keeps the client and property levels note-derived, which is the exclusion the S7 paragraph at :353 deliberately contrasts itself against, but makes the WO level order-derived: a property lists EVERY work order at that same client+property key, and a WO with no user note renders greyed. Trash never appears (o.deleted or o.tab trash, 68 of 724). Order within a property: ACTIVE first by status position in DEFAULT_STATUSES (src/constants.js:62), then COMPLETE by completion date, then SENT by invoice date, with dateless sent WOs at the bottom because that is the common case, not an edge (o.invoice exists on 215 of 724 orders while 614 sit in the sent tab). No per-phase collapsible sections; that idiom belongs to the WO module. Completion date is NOT a stored field and must be derived from o.history. That scan ALREADY EXISTS inside ageDaysFor (src/orders-logic.js:84), which walks history backwards for 'marked complete' or 'auto-flipped to complete' and falls back to dateCreated: extract it as a timestamp-returning helper and have ageDaysFor call it, so there is ONE completion rule and not a second one beside it. Counts on the tree stay NOTE counts, so a greyed WO counts zero. The invoice date is PRINTED on the sent row that has one, which is new chrome in the rail's treeRow in src/schedule.jsx. Existing coverage in test/journal-client-tree.test.js asserts the old notes-only WO level and must be updated in the same change, not left drifting. Assumption to confirm in review: complete and sent both sort NEWEST FIRST, matching the journal's newest-first idiom, since the spec names the sort keys but not their direction. Done-gate is npm run verify green plus a live Electron look, because this slice is observable.
  scope: src/orders-logic.js, src/constants.js, src/schedule.jsx, test/journal-client-tree.test.js
  steps: 5/5 done

SPEND
  heavy-verify runs observed: 4   scope: plan plan-2026-09-08-admin-s5-slice-j4-wo-listing-and
  OVER the plan budget of 2 by 2. Verify proportional to risk.
  Counts RUNS the PostToolUse hook saw, not tokens. A floor, never a bill.
  spawns this session: 0   (counted from agent meta files, not from a counter)

COST (tokens, derived from the transcripts; informational, not a gate)
  73 session(s), 12000 calls, 2.25B tokens, 88 spawn(s) all-time.
  a spawn here costs 18.8k before it reads anything (88 sample(s)); 1 file(s) estimates 281.2k.
  Derived from the transcripts on every run. Tokens, not dollars. No cache, no ledger file.

FINDINGS
  ledger: 164 finding(s), diff 7e17a6f8e0b04f88
  open 0 | fixed 49 | dismissed 115 | escalated 0
  dismissals: 106 architect-ruled, 9 Claude self-judged
  re-rule the self-judged ones: node scripts/architect.js retriage
...
ARCHITECT (manual: runs ONLY when invoked)
  ...
  Invoke: architect.js plan "<goal>" | triage | rule <id> | scope <file>
